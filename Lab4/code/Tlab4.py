...
#Now that fields have been solved, begin MOSAICKING MONTAGE:
dir = ('/Volumes/Untitled/Astro362/Lab4Data')
npixelsx = 1024
npixelsy= 1024
...
clust_B = np.zeros((npixelsx,npixelsy,44))
for i in range(1,2):
    clust_B[:,:,i-1] = fits.getdata(dir + '/clust_B_new/clust_B' +str(i) +'.fits')
for i in range(4,47):
    clust_B[:,:,i-3] = fits.getdata(dir + '/clust_B_new/clust_B' +str(i) +'.fits')

#Cluster in R-filter

clust_R = np.zeros((npixelsx,npixelsy,44))
for i in range(1,2):
    clust_R[:,:,i-1] = fits.getdata(dir + '/clust_R_new/clust_R' +str(i) +'.fits')
for i in range(4,47):
    clust_R[:,:,i-3] = fits.getdata(dir + '/clust_R_new/clust_R' +str(i) +'.fits')

#Cluster in V-filter

clust_V = np.zeros((npixelsx,npixelsy,44))
for i in range(1,2):
    clust_V[:,:,i-1] = fits.getdata(dir + '/clust_V_new/clust_V' +str(i) +'.fits')
for i in range(4,47):
    clust_V[:,:,i-3] = fits.getdata(dir + '/clust_V_new/clust_V' +str(i) +'.fits')
...
lan1_B = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan1_B[:,:,i-1] = fits.getdata(dir + '/lan_B/lan1_B' +str(i))

lan3_B = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan3_B[:,:,i-1] = fits.getdata(dir + '/lan_B/lan3_B' +str(i))

#R-filter read in:

lan1_R = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan1_R[:,:,i-1] = fits.getdata(dir + '/lan_R/lan1_R' +str(i))

lan3_R = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan3_R[:,:,i-1] = fits.getdata(dir + '/lan_R/lan3_R' +str(i))

#V-filter read in:

lan1_V = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan1_V[:,:,i-1] = fits.getdata(dir + '/lan_V/lan1_V' +str(i))

lan3_V = np.zeros((npixelsx,npixelsy,5))
for i in range(1,6):
    lan3_V[:,:,i-1] = fits.getdata(dir + '/lan_V/lan3_V' +str(i))

#####################################################interactive mouse clicker code:
#####################################################++++++++++++++++++++
...
for i in range(1,6):
    for j in range(1,4):
        ...
        lanstarlocB[:,i-1,j-1] = xc, yc #storage array, saved below

pickle.dump(lanstarlocB, open(dir + 'lanstarlocB.txt', 'wb')) #done!
...
#Annulus++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#Followed by aperture and SNR shit+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
g = 1.37 #gain (electrons)
RN = 8.2 #read noise
...
#we need the instrumental magnitudes first, then we can use the SIMBAD values to calculate the zeropoints
